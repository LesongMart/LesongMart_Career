  ...
  <div class="main-container" style="background-color: #f0f0f0; padding: 20px; padding-top: 40px;">
    <div class="pb-20">
          ...
          <div class="clearfix">
            ...
            <div class="pull-right">
              ...
              <a href="application_history.php" class="btn btn-default btn-xs" style="width: 70px;" role="button">All</a>
               <a href="application_history_accepted.php" class="btn btn-success btn-xs" style="width: 70px;" role="button">Accepted</a>
      ...
      try {
        ...
        $stmt = $conn->prepare("select * from job_application WHERE status = 'Accepted' LIMIT $start_from, $per_page");
        $stmt->execute();
  ...
  <div style="margin-left: 40%;">
      <nav>
          ...
          try {
            $conn = new PDO("mysql:host=$servername;dbname=$dbname", $username, $password);
            $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $stmt = $conn->prepare("SELECT * FROM job_application WHERE status = 'Accepted' ");
            $stmt->execute();
            $result = $stmt->fetchAll();
            $total_records = count($result);
          }
        ...
        ?>
          ...
          <?php } else { ?>
            <li><a href="application_history_accepted.php?page=<?php echo $page-1 ?>" aria-label="Previous"><span aria-hidden="true">«</span></a></li>
          <?php
          }
          for ($i=1; $i<=$total_pages; $i++)
            if ($i == $page)
              echo "<li class=\"active\"><a href=\"application_history_accepted.php?page=$i\">$i</a></li>";
            else
              echo "<li><a href=\"application_history_accepted.php?page=$i\">$i</a></li>";
          ?>
